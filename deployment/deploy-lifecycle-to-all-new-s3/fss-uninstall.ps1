...
foreach ($region in $regions) {
	if (Is-Region-Enabled($region)) {
		...
		if ($lastExitCode -eq 0) {
			...
			"$( Get-Date ) Deleting $stackName - V$version in $region..."
			...
			if ($lastExitCode -eq 0) {
				"$( Get-Date ) Successfully removed $stackName - V$version in $region"
				$updatedCount++
			} else {
				Write-Warning "Could not remove $stackName - V$version in $region"
				$failedCount++
			}
...
if ($failedCount -gt 0) {
	Write-Warning "Could not remove $stackName in $failedCount region(s)."
} else {
	if ($installedCount -gt 0) {
		"Deleted $stackName in $installedCount region(s) successfully."
	}
	if ($updatedCount -gt 0) {
		"Deleted $stackName in $updatedCount region(s) successfully."
	}
}